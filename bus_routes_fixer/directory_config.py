...
class DirectoryConfig:
    ...
    def __new__(cls, config_path="bus_routes_fixer/config/directories.json"):
    ...
    def _initialize(self, config_path=None):
        ...
        # Assign base paths
        self.temp_files_path = current_path / directories["temp_files_path"]
        self.scenario_management_temp_files_path = Path(directories["scenario_management_temp_files_path"])
        self.scenario_management_project_path = Path(directories["scenario_management_project_path"])
        self.modifications_path = Path(self.scenario_management_temp_files_path) / "Modifications"
        self.scenarios_path = Path(self.scenario_management_temp_files_path) / "Scenarios"
        ...
        if os.path.exists(self.temp_files_path):
            for filename in os.listdir(self.temp_files_path):
                file_path = os.path.join(self.temp_files_path, filename)
                try:
                    if os.path.isfile(file_path) or os.path.islink(file_path):
                        os.remove(file_path)
                    elif os.path.isdir(file_path):
        ...
        Path(self.temp_files_path).mkdir(exist_ok=True, parents=True)
        ...
        self.scenario_management_path = self.scenario_management_temp_files_path / '00_Bus_Routes_Fixer_Test_P6_Model.vpdbx'
        ...
        self.error_message_log = self.temp_files_path / 'MessageLog.txt'
        self.working_scenario_path = self.temp_files_path / 'scenarioWorking.ver'
        self.error_scenario_fixing_routes_path = self.temp_files_path / 'scenarioErrorFixing.ver'
        self.working_scenario_delete_routes_path = self.temp_files_path / 'scenarioWorkingDeleteRoutes.ver'
        self.route_deleted_transfer_path = self.temp_files_path / 'routeDeletedTransfer.tra'
        self.route_fixed_transfer_path = self.temp_files_path / 'fixedRouteAddedTransfer.tra'
        self.debug_log_path = self.temp_files_path / 'debug.log'
        self.derived_data_log_path = self.temp_files_path / 'Notes_for_Visum_Modeller.log'
        self.list_of_modifications_since_the_1st_error_paths = []
