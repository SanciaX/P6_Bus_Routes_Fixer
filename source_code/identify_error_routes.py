...
def identify_errors(error_message_file, network_file_table_of_links, network_file_table_of_turns):
    ...
    error_nodes_class.read_error_file(error_route_list_class, node_check_list_class)
            # 1. whenever there's a new (node1, node2, error_type), append node1, node2 and error_type lists
    ...
    error_node1_checklist = node_check_list_class.check_node1()  # list of all the nodes to check
    #  return self.node1 -> self.node1[counter] = node1 -> error_node_list_class.get_error_nodes(counter1, node_num, current_route)
    error_node2_checklist = node_check_list_class.check_node2()
    error_node_type_checklist = node_check_list_class.get_error_type()
    error_nodes_check = [
        (error_node1_checklist[i], error_node2_checklist[i]) for i in range(len(error_node1_checklist))
    ]
    error_nodes_type_check = [
        (error_node1_checklist[i], error_node2_checklist[i], error_node_type_checklist[i])
        for i in range(len(error_node1_checklist))
    ]
