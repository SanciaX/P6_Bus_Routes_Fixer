...
class CheckNodePairOk:
    ...
    def check_nodes_errorlink(
        ...
        self.node12ok = 999  # self.node12ok = 999: not checked yet
        type_error = " "
    ...
    def get_nodes_errorturn(
        ...
        self.node03 = str(node03.strip())
        self.check_route_node1 = check_route_node1
        self.check_route_node3 = check_route_node3
        self.node123ok = 999  #
        type_error = " "
        for row1 in nodes_type_check: #nodes_type_check [(error_node1_checklist[i], error_node2_checklist[i], error_node_type_checklist[i])
            for row2 in nodes_type_check:
                if row1[:2] == (self.node01, self.node02) and row2[:2] == (self.node02, self.node03) :
                    type_error =  "Turn block"
                    self.node123ok = 5  # self.node12ok = 5: there's a turn block between node1 and node3
        if type_error == " ":
            with open(self.network_file_table_of_turns, "r") as fp:
                ...
                for line in lines:
                    if check_route_node1 == check_route_node3:  # otherwise there is no need to search a path between node 1 and 2
                        if line.find(";B") != -1 and line.split(";")[0] == self.node01 and line.split(";")[1] == self.node02 and line.split(";")[2] == self.node03:  # when there's a link in the current network between two neighbour nodes along a line route and the link is open to bus mode
                            self.node123ok = 1  # self.node12ok = 1: the turn from node1 to node3 via node2 is fine
            if self.node123ok != 1:
