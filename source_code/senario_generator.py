...
class ScenarioGenerator:
    def save_to_sm(self, config, new_mode_set, visum):
        ...
        modification2, mod2_path, mode2_name, mode2_id = add_modification(visum, config, code,
        ...
        modification3, mod3_path, mode3_name, mode3_id = add_modification(visum, config, code, "With no info. about deleted error routes")
        ...
        modification4, mod4_path, mode4_name, mode4_id = add_modification(visum, config,"Problematic Routes Re-added", "Have the deleted problematic routes fixed and re-added to the erroneous scenario's network")
        ...
        cur_scenario = add_scenario(visum, final_mod_set, config.scenarios_path,code)
        cur_scenario.LoadInput()
        visum.ScenarioManagement.CurrentProject.RemoveScenario(cur_scenario.AttValue("NO")-1)
